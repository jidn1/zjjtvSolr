				...
				<tr>
					<td><select onchange="setViewSize(this.value);return false;"
						name="pageSize" class="pagination-page-list">
							<option <c:if test="${sc.pageSize == 5 }">selected="selected"</c:if> value="5">5</option>
							<option <c:if test="${sc.pageSize == 10 }">selected="selected"</c:if> value="10">10</option>
...
<%-- 		<input id="userName" name="userName" value="${sc.userName }" type="hidden"/> --%>
		<input id="firstName" name="firstName" value="${user.firstName }" type="hidden"/>
		<input  value="${type }" type="hidden">
		<input name="groupName" value="${user.type }" type="hidden" id="groupName">
...
<%-- 				</c:when> --%>
<%-- 				<c:when test="${fn:contains(sessionScope.userLogin.groupId,'EXPERTS')}"> --%>
...
<%-- 				</c:when> --%>
<%-- 				<c:otherwise> --%>
<%-- 					<c:if test="${fn:contains(sessionScope.userLogin.groupId,'DRAFTER')}"> --%>
