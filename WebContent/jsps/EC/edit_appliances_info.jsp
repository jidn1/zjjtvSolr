...
<script type="text/javascript">
	/* $(document).ready(
			function() {
				CKEDITOR.replace('accuracyLevel', {
					skin : 'v2',
					...
					toolbar : [ [ 'Bold', 'Italic', 'Underline', 'Strike',
							'Subscript', 'Superscript', 'Styles', 'Format',
							'Font', 'FontSize' ] ]
				});
				CKEDITOR.replace('measuringRange', {
					skin : 'v2',
					enterMode : CKEDITOR.ENTER_BR,
					removePlugins : 'elementspath',
					resize_enabled : false,
...
function updateEquipment(){
	var path = $("#path").val();
	$("#equipmentForm").attr("action","${pageContext.request.contextPath }/equip/updateEquipment.do").submit();
	
}
function toHtml(){
	$("#equipmentForm").attr("action","${pageContext.request.contextPath }/page/toHtml1.do").submit();
}
		...
		<input type="hidden" name="realPath" value="html">
			...
			<div class="basic_info">
				...
				<a href="javascript:void(0)" class="bta_w" onclick="insertEquipment()">添加</a>
					...
					<a class="s_trw"  onclick="toHtml()" style="cursor: pointer;">HTML</a>
					<!-- href="<%=basePath%>/toHtml?file_name=edit_appliances_info.jsp&&path=jsps/EC&&realName=appliances&&realPath=html" -->
					<%-- <a class="s_trw" href="${pageContext.request.contextPath}/page/writeHtml.do" style="cursor: pointer;">HTML</a> --%>
					<a class="s_trw" href="/limsEC/html/demoToHtml.html" style="cursor: pointer;">显示页面</a>
